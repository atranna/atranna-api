...
func (r *OrganizationMemberRepository) Add(member models.OrganizationMember) (models.OrganizationMember, error) {
	...
	err := r.db.QueryRow(
		`INSERT INTO organization_members (organization_id, user_id) VALUES (?, ?) RETURNING organization_id, user_id`,
...
func (r *OrganizationMemberRepository) GetByOrganizationID(organizationID int) ([]models.OrganizationMember, error) {
	rows, err := r.db.Query(`SELECT organization_id, user_id FROM organization_members WHERE organization_id = ?`, organizationID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *OrganizationMemberRepository) GetByUserID(userID int) []models.OrganizationMember {
	rows, err := r.db.Query(`SELECT organization_id, user_id FROM organization_members WHERE user_id = ?`, userID)
	if err != nil {
		return nil
	}
	defer rows.Close()
...
func (r *OrganizationMemberRepository) Delete(organizationID int, userID int) (models.OrganizationMember, bool) {
	row := r.db.QueryRow(`SELECT organization_id, user_id FROM organization_members WHERE organization_id = ? AND user_id = ?`, organizationID, userID)
	var member models.OrganizationMember
